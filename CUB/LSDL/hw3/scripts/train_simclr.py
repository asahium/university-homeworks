...
def train_epoch(model, loader, optimizer, scaler, device):
    ...
    for (x_i, x_j), _ in tqdm(loader, desc="Training SimCLR", leave=False):
...
def find_latest_checkpoint(checkpoint_dir, prefix='simclr_model_'):
...
def main():
    ...
    logger = WandbLogger(
        ...
        config_dict={
            ...
            "learning_rate": Config.SIMCLR_LR,
            "optimizer": "Adam",
            "temperature": Config.SIMCLR_TEMPERATURE,
            "projection_dim": Config.SIMCLR_PROJECTION_DIM
        }
    )
    
    # Load data
    print("Loading data...")
    train_loader = get_stl10_dataloaders('unlabeled', Config.SIMCLR_BATCH_SIZE, 'simclr')
    val_loader = get_stl10_dataloaders('train', Config.SIMCLR_BATCH_SIZE, 'simclr')
    ...
    # Training setup - use fused optimizer for better performance
    optimizer = torch.optim.Adam(model.parameters(), lr=Config.SIMCLR_LR, fused=(Config.DEVICE.type == 'cuda'))
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=Config.SIMCLR_EPOCHS)
    scaler = torch.amp.GradScaler('cuda', enabled=(Config.DEVICE.type == 'cuda'))
    
    # Check for existing checkpoints
    latest_ckpt, start_epoch = find_latest_checkpoint(Config.CHECKPOINT_DIR, 'simclr_model_')
    ...
    for epoch in range(start_epoch, Config.SIMCLR_EPOCHS):
        ...
        if (epoch + 1) % 20 == 0:
            ...
            model_path = os.path.join(Config.CHECKPOINT_DIR, f'simclr_model_{epoch+1}.pth')
            torch.save(checkpoint, model_path)
            print(f"\nCheckpoint saved to {model_path}")
            print(f"Epoch [{epoch+1}/{Config.SIMCLR_EPOCHS}] "
                  f"Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}")
    
    # Save final model in old format for compatibility
    model_path = os.path.join(Config.CHECKPOINT_DIR, 'simclr_model.pth')
    ...
    metrics_path = os.path.join(Config.RESULTS_DIR, 'simclr_metrics.json')
