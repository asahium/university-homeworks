...
class TextDataset(Dataset):
    ...
    def __init__(self, data_file: str, train: bool = True, sp_model_prefix: str = None,
        ...
        """
        train_texts, val_texts = train_test_split(texts, test_size=TextDataset.VAL_RATIO,
                                                  random_state=TextDataset.TRAIN_VAL_RANDOM_SEED)
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, int]:
        ...
        indices = torch.nn.functional.pad(
            torch.Tensor(indices), (0, self.max_length - length), "constant", self.pad_id
        )
